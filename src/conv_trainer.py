...
# freeze_emb = True
# peft_on = False
# warmup_proportion = 0.06 # peft/conv 0.03=0.799/0.879 ; 0.06=0.83/0.879

# fd="/home/yuni/prompt/causal-prompt/dataset/k-shot/comagc/16-2/"
# # data_files = {"train": fd+"train.jsonl", "test": fd+"test.jsonl"}
# # dataset = load_dataset("json", data_files=data_files)

# data_files = {"train": fd+"train.tsv", "test": fd+"test.tsv"}
# dataset = load_dataset("csv", data_files=data_files, delimiter='\t')
# # Rename the 'label' or relation column to 'labels' which is the expected name for labels by transformer library models
# # and convert the labels to integer (needed)
# dataset = dataset.map(
#     lambda x: {"labels": [int(label) for label in x["label"]]},
#     batched=True,
#     num_proc=1,
# )

# experiment_id = 'test'
# model_id = f"{experiment_id}_{dataset_name}_{model_name_or_path}".replace("/", "_")
...
#     tokenized_input = tokenizer(examples["sentence"], truncation=True, padding=True, max_length=max_length)
#     return tokenized_input
...
#     batched=True,
#     #remove_columns=dataset["train"].column_names,
#     # remove_columns=["e1", "e2", "relation", "evs"], #jsonl
#     remove_columns=["e1", "e2", "label"], #tsv
#     load_from_cache_file=False,
#     desc="Preprocess" )
...
# eval_dataloader = DataLoader(eval_dataset, shuffle=False, collate_fn=default_data_collator, batch_size=1)

# model = AutoModelForSequenceClassification.from_pretrained(model_name_or_path, return_dict=True)
# print (f'(ORIG) trainable parameters: {count_parameters(model)}')
# model.config.pad_token_id = tokenizer.pad_token_id 
...
#         prompt_tuning_init_text="Classify the relation between the gene and disease in the text.",
...
# print (f'(END) trainable parameters: {count_parameters(model)}')

# optimizer = torch.optim.AdamW(model.parameters(), lr=lr)
# lr_scheduler = get_linear_schedule_with_warmup(
#     optimizer=optimizer,
#     num_warmup_steps= warmup_proportion * (len(train_dataloader) * num_epochs),
...
#         print (f'New test best f1: {ep_f1}')
#         best_pred = ep_pred
#         best_ref = ep_ref

# print (f'BEST TEST: {best_f1}')
# print (best_pred, best_ref)
# # f1_bi = metric.compute(references=best_ref, predictions=best_pred, pos_label=1, average="binary")['f1']
# # print (f'just check: {f1_bi}')
